...
export default {
  ...
  methods: {
    ...
    fileUpload(file, index) {
      ...
      //文件存储路径 路径文件可自定义
      let storeAs = `${this.path}/${realFileName}${fileExten}`;
      this.Oss = new OSS.Wrapper(this.ossConfig);
      this.Oss.multipartUpload(storeAs, file, {
        progress: (p) => {
          return (done) => {
            this.picList[index].progress = Math.floor(p * 100);
            done();
          }
        },
        partSize: 100 * 1024
      }).then((res) => {
        this.picList[index].link = res.name;
